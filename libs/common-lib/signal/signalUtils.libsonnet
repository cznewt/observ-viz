// Core PromQL expr/legend transform logic (ported verbatim from grafana
// common-lib/common/signal/utils.libsonnet). Datasource-agnostic string work:
// produces templates that still contain %(...)s tokens; the caller applies
// `% vars`. generateUnits lives in ./unit.libsonnet.
local unit = import './unit.libsonnet';
{
  ...
  generateUnits(type, unitArg, rangeFunction):: unit.generateUnits(type, unitArg, rangeFunction),
}
