// The base signal object (ported from grafana common-lib/common/signal/base.libsonnet).
// All renderers (asTimeSeries/asStat/asTable/asGauge/asStatusHistory/asTarget/
// asTableTarget/asTableColumn/asPanelMixin/asPanelExpression/asRuleExpression) and
// all modifiers (withX). Renders via observ-viz v2 builders through ./g.libsonnet.
//
// std.get/std.member/xtd.array.slice are avoided (C++ _jsonnet binding); pure
// jsonnet helpers are used instead.
...

{
  new(
      ...
      std.uniq(  // keep unique only
        std.sort(
          std.foldl(function(total, source)
            ...
            + std.prune([objGet(source, 'infoLabel')])
                    , sourceMaps, init=[]),
        )
      ),
    ...
    vars::
      vars
      {
        ...
        aggLegend: utils.labelsToPanelLegend(
          // keep last label
          utils.sliceTail(legendLabels, -1)
        ),
    ...
    getValueMappings(sourceMaps)::
      std.uniq(
        std.foldl(function(total, source) total + objGet(source, 'valueMappings', []), sourceMaps, init=[])
      ),
      ...
      self.combineUniqueExpressions(
        [
          ...
          signalUtils.wrapExpr(
            type,
            source.expr,
            exprWrappers=objGet(source, 'exprWrappers', []),
            q=objGet(this, 'quantile', 0.95),
