...
Dashboard uid `observ-viz-svc-mimir` · 85 signals · 27 alerts · 15 recording rules.
...
| `systemd_tasks` | short | `node_systemd_unit_tasks_current{name=~"mimir.service", instance=~"$host"}` | — |
| `systemd_tasksMax` | short | `node_systemd_unit_tasks_max{name=~"mimir.service", instance=~"$host"}` | — |
| `systemd_tasksUtil` | percent | `100 * node_systemd_unit_tasks_current{name=~"mimir.service", instance=~"$host"} / clamp_min(node_systemd_unit_tasks_max{name=~"mimir.service", instance=~"$host"}, 1)` | — |
| `systemd_uptime` | s | `time() - node_systemd_unit_start_time_seconds{name=~"mimir.service", instance=~"$host"}` | — |
