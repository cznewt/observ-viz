...
Dashboard uid `observ-viz-svc-grafana-test` · 123 signals · 28 alerts · 15 recording rules.
...
| `systemd_tasks` | short | `node_systemd_unit_tasks_current{name=~"grafana-test.service", instance=~"$host"}` | — |
| `systemd_tasksMax` | short | `node_systemd_unit_tasks_max{name=~"grafana-test.service", instance=~"$host"}` | — |
| `systemd_tasksUtil` | percent | `100 * node_systemd_unit_tasks_current{name=~"grafana-test.service", instance=~"$host"} / clamp_min(node_systemd_unit_tasks_max{name=~"grafana-test.service", instance=~"$host"}, 1)` | — |
| `systemd_uptime` | s | `time() - node_systemd_unit_start_time_seconds{name=~"grafana-test.service", instance=~"$host"}` | — |
