...
def main():
    ...
    reg = registry()
    live = live_metrics()
    lines = ["# Collectors & metrics", "",
             "Generated by `scripts/gen-collector-docs.py` from "
             "`libs/common-lib/collectors.libsonnet`" +
             (" + the live metric inventory." if live else " (no live inventory available)."), ""]
    ...
    for name in sorted(reg):
        ...
        ms = per.get(name, [])
        if ms:
            lines += ["<details><summary>%d live metrics</summary>" % len(ms), ""]
            lines += ["- `" + m + "`" for m in ms]
            lines += ["", "</details>", ""]
    ...
    for lib in sorted(sigmap):
        ...
        for sname, mets, cols in sigmap[lib]:
            lines += ["| " + sname + " | " + "<br>".join("`" + m + "`" for m in mets)
                      + " | " + ", ".join(cols) + " |"]
        lines += [""]
    out = os.path.join(ROOT, "docs", "collectors.md")
    open(out, "w").write("\n".join(lines) + "\n")
    n_live = sum(len(v) for v in per.values()) if live else 0
    print("wrote %s: %d collectors, %d live metrics, %d libs cross-referenced"
          % (out, len(reg), n_live, len(sigmap)))
