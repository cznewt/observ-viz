...
{
  // new(config, members)
  //   config:  { uid, title, datasource?, selector?, tags?, owner?, domain? }
  //   members: [ { key, pack, config? } ]  (pack = an observ-viz pack object)
  new(config, members)::
    local cfg = {
    ...
    local folder = { uid: 'observ-viz-scn-' + cfg.uid, title: cfg.title };
